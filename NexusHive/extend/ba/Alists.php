...
class Alists
{
    ...
    protected $type = 'djiapi';//方便调用时获取不同的权限
 
    public function __construct($type)
    {
        $this->type = $type;
        // 从 env 读取配置，使用原有硬编码值作为默认值
        $this->url             = env('alists.url', '');
        $this->accessKeyId     = env('alists.access_key_id', '');
        $this->accessKeySecret = env('alists.access_key_secret', '');
        $this->roleArn         = env('alists.role_arn', '');
        $this->roleSessionName = env('alists.role_session_name', '');
        $this->durationSeconds = env('alists.duration_seconds', '');
