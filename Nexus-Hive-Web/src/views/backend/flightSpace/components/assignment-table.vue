                    ...
                    <el-table-column prop="status" label="执行状态" width="160">
                        <template #default="scope">
                            <div class="execute-status">
                                ...
                                <div>{{ `(${scope.row['now_point'] || 0}/${scope.row['total_point']})` }}</div>
                                <!-- 原因 -->
                                <el-tooltip
...
let newButton: OptButton[] = [
    {
        ...
        click: (row: TableRow, field: TableColumn) => {
            ...
            disposition.setDjiDockVideoId(gateway_sn.value)
            disposition.setDeviceVideoId(deviceOsds.value[gateway_sn.value].sub_device.device_sn)
        },
...
const baTable = new baTableClass(
    new baTableApi('/admin/Flighttask/'),
    {
        ...
        column: [
            ...
            // 操作
            { label: t('Operate'), align: 'center', width: 80, render: 'buttons', buttons: optButtons, fixed: 'right' },
        ],
