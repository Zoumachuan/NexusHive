...
export const DJIoperations = {
    ...
    flighttaskUndo: (sn: string, data: any = {}) => {
        return mqttSdk.publish(
            `thing/product/${sn}/services`,
            JSON.stringify({ method: 'flighttask_undo', data, timestamp: Date.now(), tid: generateUUID(), bid })
        )
    },
    // 进入指令飞行
    drcModeEnter: (sn: string, data: any = {}) => {
        return mqttSdk.publish(
            `thing/product/${sn}/services`,
            JSON.stringify({ method: 'drc_mode_enter', data, timestamp: Date.now(), tid: generateUUID(), bid })
        )
    },
    // 飞行控制权抢夺
    flighAuthorityGrab: (sn: string, data: any = {}) => {
        return mqttSdk.publish(
            `thing/product/${sn}/services`,
            JSON.stringify({ method: 'flight_authority_grab', data, timestamp: Date.now(), tid: generateUUID(), bid })
        )
    },
    // 负载控制权抢夺
    payloadAuthorityGrab: (sn: string, data: any = {}) => {
        return mqttSdk.publish(
            `thing/product/${sn}/services`,
            JSON.stringify({ method: 'payload_authority_grab', data, timestamp: Date.now(), tid: generateUUID(), bid })
        )
    },
}
